...
mem <- readRDS(file=paste0(base_dir,"data/2021/berkeley_earth/Complete_TAVG_LatLong1_grid_membership_europe.rds"))
...
# HYDE datasets are decadal over our period of interest
crs <- crs(temp_sp)
res <- res(temp_sp)
years <- seq(1750,2010, by=10)
...
den <- rbindlist(pbmclapply(years, pop_frame, mc.cores=16))
test <- merge(clim_long[month==1], den[year==2000])
ggplot(data=test) + geom_point(aes(x, y, color=log(popLevel)))
ggplot(data=test) + geom_point(aes(x, y, color=climatology))
...
## Area Average
annual_area <- temp_long[, lapply(.SD, weighted.mean, w=weight, na.rm=TRUE), by = list(name, adm0_a3, year), .SDcols=vars]
## Population Average
temp_long[, weight2 := weight*popLevel]
annual_pop <- temp_long[, lapply(.SD, weighted.mean, w=weight2, na.rm=TRUE), by = list(name, adm0_a3, year), .SDcols=vars]
## Growing season
grow_area <- temp_long[month>=4 & month<=9, lapply(.SD, weighted.mean, w=weight, na.rm=TRUE), by = list(name, adm0_a3, year), .SDcols=vars]
grow_pop <- temp_long[month>=4 & month<=9, lapply(.SD, weighted.mean, w=weight2, na.rm=TRUE), by = list(name, adm0_a3, year), .SDcols=vars]
## Monthly
monthly_area <- temp_long[, lapply(.SD, weighted.mean, w=weight, na.rm=TRUE), by = list(name, adm0_a3, year, month), .SDcols=vars]
monthly_pop <- temp_long[, lapply(.SD, weighted.mean, w=weight2, na.rm=TRUE), by = list(name, adm0_a3, year, month), .SDcols=vars]

# Output
write_dta(annual_area, path = paste0(base_dir,"data/2021/berkeley_earth/temperature_1750_2020_annual_area_europe.dta"))
write_dta(annual_pop, path = paste0(base_dir,"data/2021/berkeley_earth/temperature_1750_2020_annual_pop_europe.dta"))
write_dta(grow_area, path = paste0(base_dir,"data/2021/berkeley_earth/temperature_1750_2020_growing_season_area_europe.dta"))
write_dta(grow_pop, path = paste0(base_dir,"data/2021/berkeley_earth/temperature_1750_2020_growing_season_pop_europe.dta"))
write_dta(monthly_area, path = paste0(base_dir,"data/2021/berkeley_earth/temperature_1750_2020_monthly_area_europe.dta"))
write_dta(monthly_pop, path = paste0(base_dir,"data/2021/berkeley_earth/temperature_1750_2020_monthly_pop_europe.dta"))
...
for(i in c("annual_area","annual_pop","grow_area","grow_pop")){
  ...
  ggsave(p1, paste0(base_dir, "output/graph/data_creation/map_", i, "_tavg_europe.png"))
}
