...
packages <- c('sp','rgeos','grid','ggplot2','sf','reticulate',
              ...
              'tictoc','pbmcapply','haven','exactextractr','purrr')
...
map_dir <- paste0("/home/jgs/Dropbox/research/data/maps/")
...
shp <- shapefile(paste0(map_dir,"usa/state/cb_2014_us_state_500k/cb_2014_us_state_500k.shp"))
...
# Load data --------------------------------------------------------------
# Load a single, representative file (which for nicely formatted BE data
# is any one of the) so we can determine grid membership
...
# We will take a layer from one of the post 1880 years because that is the 
...
# exactextractr is about 10x faster than extract and is supposed to be slightly more accurate
#test <- as.data.table(suppressWarnings(raster::extract(data, shp, cellnumbers=TRUE, df=TRUE, small=TRUE, weights=TRUE, normalizeWeights=FALSE)))
...
ggsave(filename=paste0(graph_dir, "temperature_centroid_match_check_tavg_monthly.pdf"))

# Drop weather: we just want the spatial membership
cell_member <- cell_member[, value:=NULL][
  , count := NULL]
...
# locations.
fname <- paste0(data_dir,"Complete_TAVG_LatLong1_grid_membership.rds")
...
fname <- paste0(data_dir,"Complete_TAVG_LatLong1_location_names.dta")
